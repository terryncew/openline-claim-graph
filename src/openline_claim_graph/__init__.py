"""OpenLine dependency-aware provenance and source-impact prototype."""

from .analysis import compare_snapshots, disagreement_report
from .benchmark import (
    build_plan,
    build_trial_payload,
    render_inventory_as_prose,
    run_receiver_command,
    score_responses,
    validate_gold,
    validate_inventory,
    validate_pack,
    validate_plan,
)
from .bundle import verify_bundle
from .comparative_benchmark import (
    COMPARATIVE_AUTHORITY_SCHEMA,
    COMPARATIVE_GOLD_SCHEMA,
    COMPARATIVE_PACK_SCHEMA,
    COMPARATIVE_PREDICTIONS_SCHEMA,
    COMPARATIVE_SCORE_SCHEMA,
    PUBLISHED_DIAGNOSTIC_SCHEMA,
    ComparativeBenchmarkError,
    build_published_diagnostic,
    create_authority,
    create_case,
    create_gold,
    create_pack,
    import_schneider_csv,
    import_van_der_vet_dot,
    parse_dot_edges,
    run_comparative,
    score_comparative,
    validate_authority as validate_comparative_authority,
    validate_gold as validate_comparative_gold,
    validate_pack as validate_comparative_pack,
    validate_predictions as validate_comparative_predictions,
    verify_predictions as verify_comparative_predictions,
    verify_published_diagnostic,
    verify_score as verify_comparative_score,
)
from .frame import (
    ADVISORY_DEVICE_TYPES,
    FRAME_POLICY_SCHEMA,
    FRAME_REPORT_SCHEMA,
    FRAME_RULESET,
    FRAME_RULESET_HASH,
    MECHANICAL_DEVICE_TYPES,
    PROHIBITED_VERDICTS,
    FrameValidationError,
    create_advisory_finding,
    create_frame_finding,
    create_frame_policy,
    create_frame_review,
    detect_mechanical_frame_devices,
    evaluate_frame_ledger,
    sign_frame_record,
    validate_frame_finding,
    validate_frame_policy,
    verify_frame_attestation,
    verify_frame_report,
)
from .frame_review import FrameReviewError, render_frame_ledger
from .frontier import (
    FRAME_AGENT_OUTPUT_SCHEMA,
    FRAME_AGENT_TASK_SCHEMA,
    FrontierAdapterError,
    call_openai_compatible,
    call_openai_responses,
    create_proposal_task,
    create_review_task,
    import_proposal_output,
    import_review_output,
    proposal_output_schema,
    review_output_schema,
    run_autonomous_frame_pipeline,
)
from .graph import (
    CORE_PROFILE,
    PROFILE_HASH,
    GraphValidationError,
    build_source,
    create_claim,
    create_projection,
    create_relation,
    create_snapshot,
    provenance_anchor,
    source_commitment,
    source_span,
    validate_snapshot,
    verify_projection,
)
from .impact import (
    ADJUDICATION_IMPACT_REPORT_SCHEMA,
    IMPACT_POLICY_SCHEMA,
    IMPACT_REPORT_SCHEMA,
    SOURCE_STATUS_EVENT_SCHEMA,
    ImpactValidationError,
    analyze_adjudication_impact,
    analyze_source_impact,
    create_impact_policy,
    create_source_status_event,
    validate_impact_policy,
    validate_source_status_event,
    verify_adjudication_impact_bundle,
    verify_adjudication_impact_report,
    verify_impact_bundle,
    verify_impact_report,
)
from .impact_review import ImpactReviewError, render_impact_review
from .receipts import (
    create_source_disclosure,
    private_key_from_hex,
    public_key_hex,
    sign_snapshot,
    verify_receipt,
    verify_source_disclosure,
)
from .review import ReviewRenderError, render_review
from .wallet import ClaimGraphWallet
from .temporal_holdout import (
    TEMPORAL_AUTHORITY_SCHEMA,
    TEMPORAL_FUTURE_SEAL_SCHEMA,
    TEMPORAL_GOLD_SCHEMA,
    TEMPORAL_PACK_SCHEMA,
    TEMPORAL_PREDICTIONS_SCHEMA,
    TEMPORAL_PUBLISHED_DIAGNOSTIC_SCHEMA,
    TEMPORAL_SCORE_SCHEMA,
    TemporalHoldoutError,
    build_published_diagnostic as build_temporal_published_diagnostic,
    create_authority as create_temporal_authority,
    create_episode as create_temporal_episode,
    create_future_record as create_temporal_future_record,
    create_future_seal as create_temporal_future_seal,
    create_gold as create_temporal_gold,
    create_pack as create_temporal_pack,
    run_temporal,
    score_temporal,
    validate_authority as validate_temporal_authority,
    validate_future_seal_for_pack,
    validate_gold as validate_temporal_gold,
    validate_pack as validate_temporal_pack,
    validate_predictions as validate_temporal_predictions,
    verify_predictions as verify_temporal_predictions,
    verify_published_diagnostic as verify_temporal_published_diagnostic,
    verify_score as verify_temporal_score,
)

__all__ = [
    "CORE_PROFILE",
    "PROFILE_HASH",
    "ADVISORY_DEVICE_TYPES",
    "FRAME_AGENT_OUTPUT_SCHEMA",
    "FRAME_AGENT_TASK_SCHEMA",
    "FRAME_POLICY_SCHEMA",
    "FRAME_REPORT_SCHEMA",
    "FRAME_RULESET",
    "FRAME_RULESET_HASH",
    "MECHANICAL_DEVICE_TYPES",
    "PROHIBITED_VERDICTS",
    "ClaimGraphWallet",
    "FrameReviewError",
    "FrameValidationError",
    "FrontierAdapterError",
    "GraphValidationError",
    "ADJUDICATION_IMPACT_REPORT_SCHEMA",
    "IMPACT_POLICY_SCHEMA",
    "IMPACT_REPORT_SCHEMA",
    "ImpactValidationError",
    "ImpactReviewError",
    "SOURCE_STATUS_EVENT_SCHEMA",
    "analyze_adjudication_impact",
    "analyze_source_impact",
    "build_source",
    "build_plan",
    "build_trial_payload",
    "compare_snapshots",
    "create_claim",
    "create_advisory_finding",
    "create_frame_finding",
    "create_frame_policy",
    "create_frame_review",
    "create_impact_policy",
    "create_projection",
    "create_relation",
    "create_snapshot",
    "create_source_status_event",
    "create_source_disclosure",
    "disagreement_report",
    "detect_mechanical_frame_devices",
    "evaluate_frame_ledger",
    "private_key_from_hex",
    "provenance_anchor",
    "public_key_hex",
    "proposal_output_schema",
    "review_output_schema",
    "create_proposal_task",
    "create_review_task",
    "import_proposal_output",
    "import_review_output",
    "call_openai_compatible",
    "call_openai_responses",
    "render_frame_ledger",
    "run_autonomous_frame_pipeline",
    "render_review",
    "render_impact_review",
    "render_inventory_as_prose",
    "ReviewRenderError",
    "sign_snapshot",
    "sign_frame_record",
    "source_commitment",
    "source_span",
    "score_responses",
    "validate_snapshot",
    "validate_frame_finding",
    "validate_frame_policy",
    "verify_projection",
    "verify_receipt",
    "verify_source_disclosure",
    "verify_bundle",
    "verify_frame_attestation",
    "verify_frame_report",
    "run_receiver_command",
    "validate_gold",
    "validate_inventory",
    "validate_impact_policy",
    "validate_pack",
    "validate_plan",
    "validate_source_status_event",
    "verify_adjudication_impact_report",
    "verify_adjudication_impact_bundle",
    "verify_impact_report",
    "verify_impact_bundle",
    "COMPARATIVE_AUTHORITY_SCHEMA",
    "COMPARATIVE_GOLD_SCHEMA",
    "COMPARATIVE_PACK_SCHEMA",
    "COMPARATIVE_PREDICTIONS_SCHEMA",
    "COMPARATIVE_SCORE_SCHEMA",
    "PUBLISHED_DIAGNOSTIC_SCHEMA",
    "ComparativeBenchmarkError",
    "build_published_diagnostic",
    "create_authority",
    "create_case",
    "create_gold",
    "create_pack",
    "import_schneider_csv",
    "import_van_der_vet_dot",
    "parse_dot_edges",
    "run_comparative",
    "score_comparative",
    "validate_comparative_authority",
    "validate_comparative_gold",
    "validate_comparative_pack",
    "validate_comparative_predictions",
    "verify_comparative_predictions",
    "verify_comparative_score",
    "verify_published_diagnostic",
    "TEMPORAL_AUTHORITY_SCHEMA",
    "TEMPORAL_FUTURE_SEAL_SCHEMA",
    "TEMPORAL_GOLD_SCHEMA",
    "TEMPORAL_PACK_SCHEMA",
    "TEMPORAL_PREDICTIONS_SCHEMA",
    "TEMPORAL_PUBLISHED_DIAGNOSTIC_SCHEMA",
    "TEMPORAL_SCORE_SCHEMA",
    "TemporalHoldoutError",
    "build_temporal_published_diagnostic",
    "create_temporal_authority",
    "create_temporal_episode",
    "create_temporal_future_record",
    "create_temporal_future_seal",
    "create_temporal_gold",
    "create_temporal_pack",
    "run_temporal",
    "score_temporal",
    "validate_temporal_authority",
    "validate_future_seal_for_pack",
    "validate_temporal_gold",
    "validate_temporal_pack",
    "validate_temporal_predictions",
    "verify_temporal_predictions",
    "verify_temporal_published_diagnostic",
    "verify_temporal_score",
]
